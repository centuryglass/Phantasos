...
--]]
	...
	thorn=knife:subclass
	"classname=thorn,sprite=82,name=thorn,equip_slot=weapon,dthrown=3,flr_mult=1,bonuses={dmin_boost=1,dmax_boost=2,venom=true}"
	...
	def_ring = equipment:subclass
		"classname=def_ring,sprite=80,name=ring of shielding,equip_slot=ring,flr_mult=5,bonuses={ac=3}"
	sight_ring=equipment:subclass
		"classname=sight_ring,sprite=81,name=ring of vision,equip_slot=ring,flr_mult=5,bonuses={can_see=always_true}"
...
function creature:hp_change(n)
	...
	if self.hp <= 0 then
		if self==you then
			self.hp,self.exp=self.hp_max,0
...
function creature:attack(c2,dmg,hitrate)
	...
	if rndint(100) > hitrate then
		...
		if dmg == 0 then
			...
			if(self.venom)c2.poison=rndint(5,3) battle_msg(" poisoned ",".")
			if dmg >= c2.hp then
				if(self==you)kills+=1
				self.exp+=c2.exp
			end
...
"classname=player,name=rogue,sprite=128,hp_max=10,hitrate=85,min_dmg=1,max_dmg=5,take_turn=always_nil,item_table={bread=1000,apple=800,meat=200,torch=300,potion=500,scroll=500,sight_ring=1000,thorn=1000}"
...
temple_floor=floor:subclass
	"classname=temple_floor,sprite=35,alt_sprite=36,item_table={knife=20,tomahawk=0,sword=0,potion=30,scroll=30,def_ring=1,spiked_armor=0,warded_armor=-4},spawn_table={kobold=900,mantid=-15,watcher=0}"
...
function build_level()
	--load pre-designed level structures
	--from memory
	local f_lists=str_to_table"addr=264e,len=638"
...
function menu:add(name,op,trn,item)
	self{name=name,
		op=op,
		turn=trn,
		item=item}
...
function menu:open()
	...
	active_menu,self.openindex,ctrl=
	self,#open_menus,menu_ctrl
	set_redraw()
end

function menu:draw()
	local pos,w =
	self.pos+(point(10,10)*self.openindex),
	19
	foreach(self.values,
	function(v)
		w=max(w,#v.name*4+(v.item and 24 or 14))
	end)
	pos.w,pos.h=
	w,max(9*#self+2,10)
	draw_border(pos)
	for i=1,#self do
		...
		print(menuitem.name,dp.x,dp.y,10)
		i+=1
	end
end
...
function equip_menu:update()
	...
	function(equip_slot)
		...
		self:add(itm and itm.name or equip_slot,
		itm and function()
			local itm_menu=menu()
			itm_menu:add("remove "..itm.name,
			function()
				itm:remove()
				self:update()
				menu_close()
			...
			itm_menu:open()
		end or always_nil,nil,
		itm or item{sprite=48+#self})
	end)
end
...
function _init()
	...
	kills,--0
	high_scores,--(0,0,0)
	equip_types--(weapon,armor,ring)
	=queue(),queue(),
	rectangle()*16,
	loading_ctrl,
	rnd_pos(always_true),
	str_to_table"addr=2c86,len=19f",
	unpack"false,0,0,0,7,1,true,true,0,{0,0,0},{weapon,armor,ring}"
...

...
__gff__

...

